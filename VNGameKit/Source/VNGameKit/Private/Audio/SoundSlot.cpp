...
void USoundSlot::PlaySound(const FString InSoundFileName, const float InVolume, const float InVolumeMultipiler ,const float InFadeTime, const bool bRestart)
{
	...
	float Volume = InVolume * FMath::Clamp(InVolumeMultipiler, 0.01f, 10.0f);

	// 再生、または音声差し替え
	if (!IsValid(AudioComponent))
	{
		SoundWave->SetNumOfPlayedFrames(0);
		CreateAudioComponent(SoundWave, Volume);
	...
	else if (!CurrentSoundFileName.Equals(InSoundFileName) || bRestart)
	{
		...
		GetWorld()->GetTimerManager().SetTimer(
			TimerHandle,
			[this, SoundWave, Volume, FadeTime]()
			{
				AudioComponent->Stop();
...
void USoundSlot::ChangeVolumeMultipiler(const float InVolumeMultipiler)
{
	OptionVolumeMultiPiler = InVolumeMultipiler;

	if (IsValid(AudioComponent))
	{
		if (AudioComponent->IsPlaying())
		{
			AudioComponent->SetVolumeMultiplier(CurrentVolume * FMath::Clamp(InVolumeMultipiler, 0.1f, 10.0f));
		}
	}
...
void USoundSlot::OnImportFinishedNative(URuntimeAudioImporterLibrary* InImporter, UImportedSoundWave* ImportedSoundWave, ERuntimeImportStatus Status)
{
	...
	if (Status == ERuntimeImportStatus::SuccessfulImport)
	{
		...
		{
			PlaySound(ImmediatelySoundName,ImmediatelySoundVolume,OptionVolumeMultiPiler,ImmediatelySoundFadeTime,bImmediateryIsRestart);
